    ...
    <nav className={`fixed top-0 inset-x-0 z-50 transition-all duration-300 ${scrolled ? (dark ? "bg-gray-950/90 backdrop-blur-lg shadow-sm border-b border-white/5" : "bg-white/90 backdrop-blur-lg shadow-sm border-b") : "bg-transparent"}`}>
      ...
      {open && (
        <div className={`md:hidden border-t p-4 space-y-3 ${dark ? "bg-gray-950 border-white/5" : "bg-white"}`}>
          {nav.links.map((l) => (
            <a key={l.href} href={l.href} className={`block text-sm py-2 ${isActive(l.href) ? (dark ? "text-white font-semibold" : "text-gray-900 font-semibold") : (dark ? "text-white/70" : "text-gray-700")}`} onClick={() => setOpen(false)}>{bi(l.label)}</a>
          ))}
          ...
          <a href="#contact" onClick={() => setOpen(false)}>
            <Button className="w-full rounded-full font-semibold" style={{ background: C.yellow, color: C.green }}>{bi(nav.ctaLabel)}</Button>
          </a>
        </div>
      )}
    </nav>
